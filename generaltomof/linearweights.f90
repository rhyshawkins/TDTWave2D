module linearweights

  use iso_c_binding
  implicit none
  
  type :: linearweights_t

     real (kind = c_double) :: value

  end type linearweights_t

  real(kind = c_double), parameter :: EPSILON = 1.0e-9

contains

  subroutine linearweights_initialise(lw)
  end subroutine linearweights_initialise

  function linear_weights_compute(double xmin, double xmax,
  ...
  end function linear_weights_compute

  function clamp(x, target)
    real(kind = c_double), intent(in) :: x, target1, target2

    real(kind = c_double) :: clamp
    
    if (dabs(x - target) .lt. EPSILON) then
       clamp = target
    else
       clamp = x
    end if
  ...
  function dclamp(x, target1, target2)
    real(kind = c_double), intent(in) :: x, target1, target2

    real(kind = c_double) :: dclamp
    
    if (dabs(x - target1) .lt. EPSILON) then
       dclamp = target1
    else if (dabs(x - target2) .lt. EPSILON) then
       dclamp = target2
    else
       dclamp = x
    end if
